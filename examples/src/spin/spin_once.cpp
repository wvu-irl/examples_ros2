 ...
 * @section DESCRIPTION
 *
 * This is an example script for ROS2 spin once
 */
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<rclcpp::Node> spin_node = std::make_shared<SpinNode>();
    executor.add_node(spin_node);
    executor.spin_once();
    executor.spin_once();

    executor.spin_once();

    RCLCPP_INFO(spin_node->get_logger(), "Throttle also has a SKIPFIRST_THROTTLE option");
